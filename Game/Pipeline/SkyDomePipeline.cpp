...
SkyDomePipeline* SkyDomePipeline::GetInstance() {
	static SkyDomePipeline instance;
	return &instance;
}

void SkyDomePipeline::Init(DirectXCommon* dxCommon) {
...
void SkyDomePipeline::CreateGraphicsPipeline() {
	...
	// Shaderをコンパイルする
	vertexShaderBlob_ = dxCommon_->GetDxCompiler()->CompileShader(L"resources/Shader/SkyDome.vs.hlsl", L"vs_6_0");
	pixelShaderBlob_ = dxCommon_->GetDxCompiler()->CompileShader(L"resources/Shader/SkyDome.ps.hlsl", L"ps_6_0");
...
void SkyDomePipeline::CreateRootSignature() {
	HRESULT hr = 0;
	// RootSignatureを作成
...
void SkyDomePipeline::PreDraw(ID3D12GraphicsCommandList* commandList) {
	// RootSignatureを設定
...
void SkyDomePipeline::Draw(ID3D12GraphicsCommandList* commandList, const ViewProjection& _viewProjection, SkyDome* _skyDome) {

	_skyDome->GetObj3d()->UpdateWVPData(_viewProjection);
	D3D12_VERTEX_BUFFER_VIEW vbView = _skyDome->GetObj3d()->GetModel()->GetVBV();
	D3D12_INDEX_BUFFER_VIEW  ibView = _skyDome->GetObj3d()->GetModel()->GetIBV();

	/// Set VertexBufferView
	commandList->IASetVertexBuffers(0, 1, &vbView);
	commandList->IASetIndexBuffer(&ibView);

	/// TRANSFORM
	Matrix4x4 matWorld = _skyDome->GetObj3d()->transform_.matWorld_;
	Matrix4x4 matWVP = matWorld * _viewProjection.matView_ * _viewProjection.matProjection_;
	transformBuffer_.SetMappedData(matWVP);
	transformBuffer_.BindForGraphicsCommandList(commandList, ROOT_PARAM_TRANSFORM);

	/// TEXTURE SET
	uint32_t texHandle = TextureManager::GetInstance()->LoadTexture("./resources/Model/skydome/Hexagon.png");
	D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = TextureManager::GetInstance()->GetTextureHandle(texHandle);
	commandList->SetGraphicsRootDescriptorTable(
		ROOT_PARAM_TEXTURE, gpuHandle
	);

	/// Time SET
	timeBuffer_.SetMappedData(timeBuffer_.GetMappingData() + Frame::DeltaTime());
	timeBuffer_.BindForGraphicsCommandList(commandList, ROOT_PARAM_TIME);


	/// DRAW
	commandList->DrawIndexedInstanced(ibView.SizeInBytes / sizeof(uint32_t), 1, 0, 0, 0);
