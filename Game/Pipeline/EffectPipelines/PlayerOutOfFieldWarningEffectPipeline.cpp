#include "PlayerOutOfFieldWarningEffectPipeline.h"
...
PlayerOutOfFieldWarningEffectPipeline* PlayerOutOfFieldWarningEffectPipeline::GetInstance() {
	static PlayerOutOfFieldWarningEffectPipeline instance;
	return &instance;
}

void PlayerOutOfFieldWarningEffectPipeline::Init(DirectXCommon* dxCommon) {
...
void PlayerOutOfFieldWarningEffectPipeline::CreateGraphicsPipeline() {
	...
	// Shaderをコンパイルする
	vertexShaderBlob_ = dxCommon_->GetDxCompiler()->CompileShader(L"resources/Shader/PlayerOutOfFieldWarningEffect.vs.hlsl", L"vs_6_0");
	pixelShaderBlob_ = dxCommon_->GetDxCompiler()->CompileShader(L"resources/Shader/PlayerOutOfFieldWarningEffect.ps.hlsl", L"ps_6_0");
...
void PlayerOutOfFieldWarningEffectPipeline::CreateRootSignature() {
	HRESULT hr = 0;
	// RootSignatureを作成
...
void PlayerOutOfFieldWarningEffectPipeline::PreDraw(ID3D12GraphicsCommandList* commandList) {
	// RootSignatureを設定
...
void PlayerOutOfFieldWarningEffectPipeline::Draw(ID3D12GraphicsCommandList* _cmdList, GameController* _gameController) {
	if (!_gameController) {
		return;
	}

	if (!_gameController->GetIsPlayerOutOfField()) {
		return;
	}
	...
	effectBuffer_.SetMappedData(Vector4{ 0.75f, 0.1f, 0.0f, 1 });
