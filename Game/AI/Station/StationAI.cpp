...
void StationAI::ShowGui(){
	if (!ImGui::CollapsingHeader("StationAI",ImGuiTreeNodeFlags_DefaultOpen)) return;

	// 攻撃比率
	ImGui::Text("Attack Ratio: %.3f",out_.attackRatio);
	ImGui::ProgressBar(out_.attackRatio,ImVec2(0.f,0.f));

	// 割当て数
	ImGui::Text("Desired Attackers: %d",out_.desiredAttackers);
	ImGui::Text("Desired Defenders: %d",out_.desiredDefenders);

	// 直近フレームの特徴量
	if (ImGui::TreeNode("Features / Signals")){
		ImGui::Text("S  (self low HP)   = %.3f",dbg_.S);
		ImGui::Text("R  (rival low HP)  = %.3f",dbg_.R);
		ImGui::Text("H  (home threat)   = %.3f",dbg_.H);
		ImGui::Text("D  (dist/ref)      = %.3f  (dist=%.1f)",dbg_.D,dbg_.dist);
		ImGui::Text("Sd (self dmg rate) = %.3f",dbg_.Sd);
		ImGui::Text("Rd (rival dmg rate)= %.3f",dbg_.Rd);
		ImGui::TreePop();
	}

	// ユーティリティとシグモイド
	if (ImGui::TreeNode("Utilities / Sigmoid")){
		ImGui::Text("U_attack = %.3f",dbg_.U_attack);
		ImGui::Text("U_defend = %.3f",dbg_.U_defend);
		ImGui::Text("pAtk     = %.3f",dbg_.pAtk);
		ImGui::TreePop();
	}
}
...
void StationAI::UpdateWeighted(float dt,
	...
	dbg_.Sd = Sd;
	dbg_.Rd = Rd;
	dbg_.U_attack = U_attack;
	dbg_.U_defend = U_defend;
	dbg_.pAtk = pAtk;
	dbg_.dist = dist;
	...
	if (replanT_ <= 0.0f){
		...
		if (std::fabs(newRatio - out_.attackRatio) >= cfg_.ratioDeadband){ ApplyQuotas(selfPos,newRatio); }
	}
}
...
void StationAI::ApplyQuotas(const Vector3& defendAt, float ratio){
	...
	// ★追加：攻撃対象が居る場合は最低1体を攻撃に回す
	if (rivalCached_ != nullptr && total > 0){ defenders = std::min(defenders,total - 1); }
