#include "BreakBoundaryMission.h"
...
// ========================================
BreakBoundaryMission::BreakBoundaryMission() {
    // デフォルト値を設定
    maxWaitTime_       = 0.3f;
...
void BreakBoundaryMission::OnMissionStart() {
...
void BreakBoundaryMission::OnMissionUpdate() {
    switch (currentStep_) {
    case MovementStep::EXPLAIN:
        if (ProcessStep(maxWaitTime_)) {
            ...
            isInputActive_    = false;
            pPlayer_->GetBulletShooter()->SetIsBreakBoundary(false);
        }
        break;

    case MovementStep::WAIT_INPUT:
        if (pPlayer_) {
            bool inputConditionMet = pPlayer_->GetBulletShooter()->GetIsBoundaryBreak();
            ...
            if (inputConditionMet) {
                if (!isInputActive_) {
                    ...
                    // プログレス更新（2回押しで100%）
                    float inputProgress = std::clamp(currentInputTime_, 0.0f, 1.0f);
                    SetProgress(inputProgress);

                    // 2回押したら成功
                    if (currentInputTime_ >= 1.0f) {
...
void BreakBoundaryMission::OnMissionComplete() {
    // ミッション完了時の処理
    showGauge_ = false;
}

void BreakBoundaryMission::BindParams() {
...
void BreakBoundaryMission::AdjustParam() {
    BaseTutorialMission::AdjustParam();
}

void BreakBoundaryMission::AdjustUniqueParam() {
#ifdef _DEBUG
    // 基本パラメータ
