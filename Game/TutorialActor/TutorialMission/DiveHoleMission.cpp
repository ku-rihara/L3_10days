...
// ========================================
DiveHoleMission::DiveHoleMission() {
    // デフォルト値を設定
    maxWaitTime_       = 0.3f;
...
void DiveHoleMission::OnMissionStart() {
...
void DiveHoleMission::OnMissionUpdate() {
    ...
    case MovementStep::WAIT_INPUT:
        if (pPlayer_) {
            bool inputConditionMet = pPlayer_->GetWorldPosition().y <= -10.0f;
...
void DiveHoleMission::OnMissionComplete() {
    // ミッション完了時の処理
    showGauge_ = false;
}

void DiveHoleMission::BindParams() {
...
void DiveHoleMission::AdjustParam() {
    BaseTutorialMission::AdjustParam();
}

void DiveHoleMission::AdjustUniqueParam() {
#ifdef _DEBUG
    // 基本パラメータ
