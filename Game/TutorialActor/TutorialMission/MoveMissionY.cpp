...
void MoveMissionY::OnMissionStart() {
...
void MoveMissionY::OnMissionUpdate() {
    ...
    case MovementStep::WAIT_INPUT:
        if (pPlayer_) {
            // 入力チェック
            float inputValue       = std::fabsf(pPlayer_->GetAngleInput().y);
...
void MoveMissionY::OnMissionComplete() {
    // ミッション完了時の処理
    showGauge_ = false;
}

void MoveMissionY::BindParams() {
...
void MoveMissionY::AdjustParam() {
    BaseTutorialMission::AdjustParam();
}

void MoveMissionY::AdjustUniqueParam() {
#ifdef _DEBUG
    // 基本パラメータ
