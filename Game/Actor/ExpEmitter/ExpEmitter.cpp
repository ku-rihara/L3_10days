...
void ExpEmitter::Init() {
    ...
    npcHomingBulletEmitter_.reset(ParticleEmitter::CreateParticlePrimitive("EnemyMissile", PrimitiveType::Box, 9000));
...
void ExpEmitter::HormingUpdate() {
    npcHomingBulletEmitter_->Update();
}
void ExpEmitter::HormingEmitter(const Vector3& pos, const Vector3& rotate) {
    npcHomingBulletEmitter_->SetTargetPosition(pos);
    npcHomingBulletEmitter_->SetEmitterRotate(rotate);
    npcHomingBulletEmitter_->SetTargetRotate(rotate);
    npcHomingBulletEmitter_->Emit();
}