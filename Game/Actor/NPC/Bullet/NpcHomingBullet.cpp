...
static Vector3 RotateTowards(const Vector3& dir, const Vector3& desired, float maxRad) {
    ...
    if (ang <= 1e-6f) return d1;           // ほぼ一致
    if (maxRad <= 0.0f) return d0;         // 回せない

    // 目標角の方が小さければ一気に合わせる
    if (ang <= maxRad) return d1;
...
void NpcHomingBullet::Init() {
    ...
    if (turnRateDegPerSec_ <= 0.0f)   turnRateDegPerSec_ = 360.0f;   // 1秒で360°
    if (loseAngleDeg_ <= 0.0f)        loseAngleDeg_ = 120.0f;        // 後方120°超でロスト
    if (loseDistance_ <= 0.0f)        loseDistance_ = 5000.0f;       // ある程度遠距離まで追従
